...
[TestClass]
public class ErrorPageTests : PageTest
{
    // test class variables shared across all tests
    private const string Port = "5085";
    private static readonly Uri Url = new($"http://localhost:{Port}/Error");
    private static readonly TimeSpan ClickDelayTime = TimeSpan.FromMilliseconds(100);
    ...
    public async Task ErrorPage_Should_RenderWithoutJavaScriptErrors()
    {
        ...
        Assert.AreEqual(0, consoleMessages.Count(m => m.Type == "error"));
    }
}